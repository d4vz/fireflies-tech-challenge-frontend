...
[TanStack Query](https://tanstack.com/query/latest) manages client-side server state. It gives us query keys, caching, stale times, and refetching without writing the same fetch logic in every component. Meeting lists poll every two seconds while a meeting is queued or processing, then stop polling when the work is done.
